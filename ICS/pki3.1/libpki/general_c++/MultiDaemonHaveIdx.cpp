...
#include "MultiDaemonHaveIdx.hpp"
#include "Trace.h"

#ifdef __CYGWIN__
#define TRACEFILE "/cygdrive/c/multidaemonidx.log"
#else
#define TRACEFILE "/tmp/multidaemonidx.log"
#endif
...
MultiDaemonHaveIdx::MultiDaemonHaveIdx()
{
  _idxProc = 0;
}
...
void MultiDaemonHaveIdx::registerProcess(int port, int (MultiDaemonHaveIdx::*proc)(int idx))
{
  _ports.push_back(port);
  _procs[_idxProc++] = proc;
}

void MultiDaemonHaveIdx::_callProcess(int idx)
{
...
void MultiDaemonHaveIdx::start(string parentExitMsg)
{
  beforeDaemonize();
  _bindTest();
...
// 소켓을 바인드할 수 있는지 체크한다.
void MultiDaemonHaveIdx::_bindTest()
{
...
void MultiDaemonHaveIdx::_listen()
{
  vector<int>::size_type i;
